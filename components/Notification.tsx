...
export const Notification: React.FC<NotificationProps> = ({ notification, setNotification }) => {
  ...
  const baseClasses = 'fixed p-4 rounded-lg shadow-lg text-white max-w-sm z-50 transition-opacity duration-300 cursor-pointer';
  const typeClasses = {
  ...
  return (
    <div className={`${baseClasses} ${positionClasses[notification.type]} ${typeClasses[notification.type]}`} onClick={() => setNotification(null)}>
